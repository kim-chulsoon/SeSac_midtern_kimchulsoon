...
const del = document.querySelector("btnX");
...
function addTodo() {
  ...
  newTodo.className = "todo todo3";
  const newList = document.createElement("li");
  newList.textContent = input.value;
  const newBtn = document.createElement("li");
  newBtn.className = "btnX";

  newTodo.appendChild(document.querySelector("todolist"));
  newList.appendChild(newTodo);
  newBtn.appendChild(newTodo);
  input.value = "";
}
...
function deleteTodo(item) {
  let removOne = item.parentElement;
  removOne.remove();
}
